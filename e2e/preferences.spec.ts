...
test.afterEach(async ({ page }) => {
  await page.evaluate(() =>
    window.localStorage.removeItem("wo-video-converter-customPresets"),
  );
});
...
test.describe("Preferences", () => {
  test("import custom presets", async ({ page }) => {
    ...
    await page
      ...
      .setInputFiles(path.join(__dirname, "video-converter-presets.json"));

    // check preview
    await expect(page.getByText("video-converter-presets.json")).toBeVisible();
...
async function addPresets(page) {
  await page.evaluate(() => {
    window.localStorage.setItem(
      "wo-video-converter-customPresets",
      JSON.stringify([
        {
          id: "mp3 2",
          label: "mp3 2",
